...
% --- Broad context: malware and the two-component architecture ---
Malware remains one of the most persistent threats in cybersecurity, with adversaries continuously developing new techniques to compromise systems and evade detection~\cite{mitre_attack}.
At the implementation level, modern malware that delivers arbitrary code payloads typically exhibits a two-component architecture: the \textit{payload}---the shellcode that performs the intended malicious action, such as establishing a reverse shell or deploying ransomware---and the \textit{loader}---the program responsible for preparing, placing, and executing that payload in memory.
While the payload defines \textit{what} the malware does, the loader determines \textit{how} the payload reaches execution, handling memory allocation, decryption, and control flow transfer.

% --- Narrow context: why loaders are the evasion surface ---
The loader is where the majority of evasion engineering concentrates.
Security products---antivirus (AV) engines and Endpoint Detection and Response (EDR) systems---primarily interact with loader behavior: scanning for encrypted content at rest, monitoring memory allocation calls, and detecting suspicious thread creation or code injection~\cite{mitre_execution}.
The payload itself is often generic and interchangeable; it is the loader that determines whether a given payload successfully executes or is detected.
This paper focuses specifically on \textit{shellcode loaders}---programs that load and execute position-independent code (shellcode) in memory---as distinct from DLL loaders, script-based loaders, or fileless techniques.
Shellcode loaders represent the most prevalent class in current threat landscapes while exhibiting sufficient internal complexity to warrant structured decomposition.

% --- Problem statement: fragmented knowledge ---
Despite the central role of loaders, the current landscape presents significant challenges for both researchers and defenders.
On the offensive research side, knowledge is \textbf{fragmented}: techniques such as process injection~\cite{process_injection}, API hook evasion~\cite{api_unhooking}, and direct syscall invocation~\cite{hellsgate} are scattered across blog posts, open-source repositories, and conference talks, with no unifying framework that relates them within a loader's execution lifecycle.
On the defensive side, security products treat loaders as \textbf{opaque binaries}---a defender investigating why a loader evaded detection cannot easily determine whether evasion succeeded due to encrypted storage, direct syscall invocation, or callback-based execution, each of which requires a different detection strategy.

% --- Brief gap motivation (detailed review in Section 2) ---
Existing approaches address aspects of this problem but leave critical gaps.
Threat modeling frameworks such as MITRE ATT\&CK~\cite{mitre_attack} decompose adversary behavior at the tactic and technique level but do not model the \textit{internal functional stages} of a single loader binary.
Automated analysis platforms such as Cuckoo Sandbox~\cite{cuckoo} and ANY.RUN~\cite{anyrun} execute samples as black boxes without supporting systematic technique-level comparison.
A detailed review of related work is provided in Section~2.

% --- This paper's proposal ---
This paper addresses these gaps by proposing a \textbf{six-stage pipeline model} that decomposes shellcode loaders into discrete functional stages---Anti-Analysis, Storage, Allocation, Transformation, Writing, and Execution---and building upon this model a \textbf{dual-purpose platform} that serves both offensive and defensive security research.
Our contributions are:

\begin{enumerate}[leftmargin=*, itemsep=2pt]
    \item A \textbf{six-stage pipeline model} that decomposes shellcode loaders into discrete, composable functional stages, providing a standardized vocabulary for description, classification, and comparison of loader implementations.

    \item A \textbf{dual-purpose platform} built on this model that bridges offensive and defensive research:
    \begin{itemize}[leftmargin=*, itemsep=1pt]
        \item \textit{Red team}: structured loader generation through modular, per-stage technique selection, enabling systematic study without fragmented knowledge sources.
        \item \textit{Blue team}: stage-aware detection surface mapping with concrete Sigma and YARA rules, and a methodology for identifying detection coverage gaps.
    \end{itemize}

    \item An \textbf{automated testing infrastructure} that compiles loader variants through compile-time technique composition, executes them in isolated VM environments against security products, and collects structured telemetry mapped to pipeline stages for detection analysis.
\end{enumerate}

% --- Key findings preview ---
Experimental validation against Windows Defender demonstrates that the Transformation stage is the primary detection differentiator, with AES encryption eliminating static detection entirely, and that stage-specific detection rules provide visibility even for configurations that evade commercial AV---confirming the value of stage-level analysis for both attack research and defense engineering.

% --- Paper organization ---
The remainder of this paper is organized as follows.
Section~2 reviews related work and identifies the research gap.
Section~3 presents the methodology, encompassing the pipeline model and the dual-purpose platform.
